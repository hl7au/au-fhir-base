...
<xsl:apply-templates select="f:differential/f:element[ fn:position() != 1]"/>
  ...
  <xsl:template match="f:element">
    ...
    <xsl:if test="(f:sliceName or ( (f:slicing or count($parts)=2) and not(contains(f:path/@value,'.extension')))) ">
      ...
      <xsl:if test="not(f:sliceName) or f:type/f:code[@value='Reference']">1. <xsl:apply-templates select="//f:snapshot/f:element[@id = $idvalue]" mode="card"/><xsl:text> </xsl:text>&lt;span style='color:green'&gt;<xsl:value-of select="replace(f:path/@value, $rname, '')"/>&lt;/span&gt;<xsl:text> </xsl:text><xsl:value-of select="f:short/@value"/><xsl:if test="f:type/f:code/@value='Reference' and f:short/@value and not(f:slicing)"> (<xsl:for-each select="f:type[f:code/@value='Reference' and f:targetProfile]"><xsl:if test="position()!=1"> \| </xsl:if><xsl:value-of select="tokenize(f:targetProfile/@value, '/')[position()=last()]"/></xsl:for-each>)</xsl:if></xsl:if>
  ...
  <xsl:template match="f:element" mode="ext">
    <xsl:variable name="idvalue" select="@id"/>
   <xsl:variable name="parts" select="fn:tokenize(f:path/@value,'\.')"/>
    <xsl:if test="f:sliceName and $parts[count($parts)] = 'extension'"><xsl:text>&#x20;&#x20;&#x20;&#x20;</xsl:text>
  ...
  <xsl:if test="not(f:short)">* <xsl:apply-templates select="//f:snapshot/f:element[@id = $idvalue]" mode="card"/><xsl:text> </xsl:text><xsl:value-of select="f:sliceName/@value"/> extension</xsl:if><xsl:text>
</xsl:text>
    </xsl:if>
