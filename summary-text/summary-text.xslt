...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:f="http://hl7.org/fhir">
  ...
  <xsl:template match="f:StructureDefinition">This profile contains the following variations from [<xsl:value-of select="f:type/@value"/>](<xsl:value-of select="replace(f:baseDefinition/@value,'http://hl7.org/fhir/StructureDefinition/','http://hl7.org/fhir/STU3/')"/>):
...
<xsl:apply-templates select="f:differential/f:element[ fn:position() != 1]"/>
  ...
  <xsl:template match="f:element">
    ...
    <xsl:if test="(f:sliceName or ( (f:slicing or count($parts)=2) and not(contains(f:path/@value,'.extension')))) ">
      <xsl:if test="not(f:sliceName)">1. <xsl:apply-templates select="//f:snapshot/f:element[@id = $idvalue]" mode="card"/><xsl:text> </xsl:text>&lt;span style='color:green'&gt;<xsl:value-of select="f:path/@value"/>&lt;/span&gt;</xsl:if>
      <xsl:if test="f:sliceName">
        <xsl:if test="not(contains(f:path/@value,'.extension'))"><xsl:text>&#x20;&#x20;&#x20;</xsl:text></xsl:if>
        <xsl:if test="f:short and ( (count($parts)=2 and $parts[count($parts)] = 'extension' ) or not(contains(f:path/@value,'.extension')))"><xsl:if test="$parts[count($parts)] = 'extension' and count($parts)=2">1. <xsl:apply-templates select="//f:snapshot/f:element[@id = $idvalue]" mode="card"/></xsl:if><xsl:if test="not(contains(f:path/@value,'.extension'))">* <xsl:apply-templates select="//f:snapshot/f:element[@id = $idvalue]" mode="card"/></xsl:if><xsl:text> </xsl:text><xsl:value-of select="f:short/@value"/>
