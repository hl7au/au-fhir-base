...
- When individual ingredient strength is unavailable as a Ratio, it can be provided using additional datatypes (CodeableConcept,Quantity) using [FHIR R5 element pre-adoption](https://hl7.org/fhir/R5/versions.html#extensions) of the [`Medication.ingredient.strength[x]`](https://www.hl7.org/fhir/R5/medication-definitions.html#Medication.ingredient) element with the extension URL [http://hl7.org/fhir/5.0/StructureDefinition/extension-Medication.ingredient.strength[x]](http://hl7.org/fhir/5.0/StructureDefinition/extension-Medication.ingredient.strength[x]). See example [Tadim](Medication-IngredientStrengthExtension0.html). It is recommended to utilise the FHIR R4 Element `Medication.ingredient.strength` when representing Ratio values and the pre-adopted R5 element (via extension) only for other required datatype representations i.e CodebleConcept for coded or text representation, Quantity for numeric quantities. 
