...
- When a structured data for `Medication.ingredient.strength` is unavailable ingredient strength can be surfaced as free-text via the backported R5 `Medication.ingredient.strengthCodeableConcept` extension:
  - this is to be interpreted as the strength of that specific ingredient, not the strength of the medication as a whole
  - the extension may be used once on each `Medication.ingredient` element
