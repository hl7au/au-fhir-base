...
- When asserting that a patient is not currently taking any medications, `MedicationStatement.medicationCodeableConcept` is used with the code SNOMED CT 787481004 &#124;No known medications&#124;:
  - `MedicationStatement.status` is active
  - `MedicationStatement.effective[x]` indicates the date/time or period when the statement is known to be true
