### Usage Notes
- To represent the environment type of an endpoint (e.g. production, staging, test), use the cross-version extension [extension-Endpoint.environmentType](http://hl7.org/fhir/5.0/StructureDefinition/extension-Endpoint.environmentType) from the [FHIR Cross-Version Extensions IG (xver-r5.r4)](https://hl7.org/fhir/uv/xver-r5.r4/0.0.1-snapshot-2/index.html). This extension backports the R5 `Endpoint.environmentType` element for use in R4.
...
- The following AU-defined terminology may be useful when populating `Endpoint.connectionType` and `Endpoint.payloadType`:
